...
Comparing startDate and endDate with i.startDate and i.endDate we can see the foverlaps generated a line for the month, but that the current intervals ended the month before or didn't even have started.
...
For clarity we have omitted the m1-suffix columns in the output. As we can see the results are correct now. The incorrect lines have vanished.
...
In our use-case or this blog the easiest solution would be eliminate these rows from our dataset, as we are only interested in the amount someone hase to pay which is time/date multiplied by the sum to pay per month. Lets see how we can do this

```{r}

result_sp2 = foverlaps(dt_special[startDate <= endDatem1], intervals, by.x = colsm1, by.y = colsm1)
knitr::kable(result_sp2[,.(startDate,endDate,name,toPayPerMonth)])
```
...
If on the other hand we are interested in conserving the information that the name-change happened without change of payment per month.

```{r}
dt_special[startDate == endDate, startDate := startDate - days(1)]
result_sp3 = foverlaps(dt_special, intervals, by.x = colsm1, by.y = colsm1)
result_sp3[startDate == endDatem1, startDate := endDate]
```
